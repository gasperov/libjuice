...
void conn_turn_tcp_connect(juice_agent_t *agent, const addr_record_t *dst) {
	...
	switch (agent->config.concurrency_mode) {
	case JUICE_CONCURRENCY_MODE_POLL:
		conn_poll_turn_tcp_connect(agent, dst);
		break;
	case JUICE_CONCURRENCY_MODE_THREAD:
		conn_thread_turn_tcp_connect(agent, dst);
		break;
	default:
...
bool conn_turn_tcp_connected(juice_agent_t *agent) {
	...
	switch (agent->config.concurrency_mode) {
	case JUICE_CONCURRENCY_MODE_POLL:
		return conn_poll_turn_tcp_connected(agent);
	case JUICE_CONCURRENCY_MODE_THREAD:
		return conn_thread_turn_tcp_connected(agent);
	default:
		return false;
	}
}
