...
static conn_mode_entry_t mode_entries[MODE_ENTRIES_SIZE] = {
    {conn_poll_registry_init, conn_poll_registry_cleanup, conn_poll_init, conn_poll_cleanup,
     ...
     conn_poll_turn_tcp_send, conn_poll_turn_tcp_connect, conn_poll_turn_tcp_connected, conn_poll_get_addrs,
     NULL, NULL, NULL, MUTEX_INITIALIZER, NULL},
    {conn_mux_registry_init, conn_mux_registry_cleanup, conn_mux_init, conn_mux_cleanup,
     conn_mux_lock, conn_mux_unlock, conn_mux_interrupt, conn_mux_send, NULL,
     NULL, NULL, NULL, conn_mux_get_addrs,
    ...
    {NULL, NULL, conn_thread_init, conn_thread_cleanup,
     ...
     conn_thread_turn_tcp_send, conn_thread_turn_tcp_connect, conn_thread_turn_tcp_connected, conn_thread_get_addrs,
     NULL, NULL, NULL, MUTEX_INITIALIZER, NULL}
};
...
int conn_turn_tcp_send(juice_agent_t *agent, const char *data, size_t size, int ds) {
	if (!agent->conn_impl)
		return -1;

	conn_mode_entry_t *entry = get_agent_mode_entry(agent);
	if (entry->turn_tcp_send_func)
		return entry->turn_tcp_send_func(agent, data, size, ds);

	JLOG_WARN("TURN TCP send not supported in this concurrency mode");
	return -1;
}
...
bool conn_turn_tcp_connected(juice_agent_t *agent) {
	...
	conn_mode_entry_t *entry = get_agent_mode_entry(agent);
	if (entry->turn_tcp_connected_func)
		return entry->turn_tcp_connected_func(agent);

	return false; // Not yet implemented in this concurrency mode
}
